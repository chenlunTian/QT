...
QString FlatUI::setPushButtonQss(QPushButton *btn, int radius, int padding, int width, int height,
...
{
    QStringList list;
    list.append(QString("QPushButton{border-style:none;padding:%1px;border-radius:%2px;color:%3;background:%4; min-width:%5; min-height:%6; }")
                .arg(padding).arg(radius).arg(normalTextColor).arg(normalColor).arg(width).arg(height));
...
QString FlatUI::setProgressQss(QProgressBar *bar, int barHeight,
                                     int barRadius, int fontSize,int width,double margin,
                                     const QString &normalColor,
                                     const QString &chunkColor)
{
    ...
    list.append(QString("QProgressBar:chunk{background-color:%1;border-radius:%2px;margin: %3px;width:%4px;}")
                .arg(chunkColor).arg(barRadius).arg(margin).arg(width));
...
QString FlatUI::setCheckBoxQss(QCheckBox *rbtn, int indicatorRadius,
                               const QString &normalColor,
                               const QString &checkColor)
{
    int indicatorWidth = indicatorRadius * 2;
    ...
    QStringList list;
    list.append(QString("QCheckBox::indicator{border-radius:%1px;width:%2px;height:%2px;}")
                .arg(indicatorRadius).arg(indicatorWidth));
    list.append(QString("QCheckBox::indicator::unchecked{background:qradialgradient(spread:pad,cx:0.5,cy:0.5,radius:0.5,fx:0.5,fy:0.5,"
                        "stop:0.6 #FFFFFF,stop:0.7 %1);}").arg(normalColor));
    list.append(QString("QCheckBox::indicator::checked{background:qradialgradient(spread:pad,cx:0.5,cy:0.5,radius:0.5,fx:0.5,fy:0.5,"
                        "stop:0 %1,stop:0.3 %1,stop:0.4 #FFFFFF,stop:0.6 #FFFFFF,stop:0.7 %1);}").arg(checkColor));
